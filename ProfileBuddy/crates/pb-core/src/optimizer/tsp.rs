...
impl TspOptimizer {
    ...
    fn identify_hotspots(&self, nodes: &[&DecodedNode]) -> Vec<NodeCluster> {
        ...
        let cluster_radius = 50.0; // yards
        let min_cluster_size = 3;

        // Simple clustering: for each node, check if it forms a cluster with nearby nodes
        ...
        for (i, &node) in nodes.iter().enumerate() {
            ...
            for (j, &other) in nodes.iter().enumerate() {
                ...
                let dist =
                    distance_2d(node.world_x, node.world_y, other.world_x, other.world_y);
                if dist <= cluster_radius {
                    cluster_nodes.push(j);
                }
            }

            // If cluster is large enough, create a hotspot
            if cluster_nodes.len() >= min_cluster_size {
                ...
                // Calculate actual radius needed
                for &idx in &cluster_nodes {
                    let dist =
                ...
                hotspots.push(NodeCluster {
                    ...
                    center_y,
                    center_z,
                    radius: max_dist + 10.0, // Add some buffer
