-- SentinelUI — Custom window framework for SentinelDuoFarm.
-- Adapted from AstroUI. Provides tab-based canvas rendering with the sentinel theme.
...
function SentinelUI:_get_window_local_mouse_pos(space_hint)
...
function SentinelUI.new(config)
    ...
    self.id = config.id or "sentinel_ui"
    self.title = config.title or "SentinelDuo Control"
    self.theme_name = config.theme or "sentinel"
    self.colors = THEMES[self.theme_name] or THEMES.sentinel
    self.sections = {}
    ...
    self.menu = {
        enable = menu_checkbox(false, "sentinel_ui_enable_" .. self.id),
        pos_x = menu_slider_int(0, 10000, config.default_x or 700, "sentinel_ui_x_" .. self.id),
        pos_y = menu_slider_int(0, 10000, config.default_y or 200, "sentinel_ui_y_" .. self.id),
        size_x = menu_slider_int(0, 10000, config.default_w or 450, "sentinel_ui_w_" .. self.id),
        size_y = menu_slider_int(0, 10000, config.default_h or 600, "sentinel_ui_h_" .. self.id),
        active_tab = menu_slider_int(0, 100, 1, "sentinel_ui_tab_" .. self.id)
    }
...
function SentinelUI:register_section(section)
...
---@class sentinel_ui_tab_builder
---@field _ui sentinel_ui
---@field _section table
local TabBuilder = {}
TabBuilder.__index = TabBuilder

---@param ui sentinel_ui
---@param section table
---@return sentinel_ui_tab_builder
...
---@param build_fn fun(t:sentinel_ui_tab_builder)
function SentinelUI:add_tab(tab, build_fn)
...
---@param tabs table[]
function SentinelUI:set_tabs(tabs)
    self.sections = {}
    if not tabs then
        return
    end
...
SentinelUI.TabBuilder = TabBuilder

-- ============================================================================
-- WINDOW MANAGEMENT
-- ============================================================================

function SentinelUI:_build_window()
...
function SentinelUI:_sync_window_state()
    if not self.window then
        return
    end
...
function SentinelUI:_is_enabled()
...
function SentinelUI:_get_key_name(key_code)
...
function SentinelUI:_is_section_visible(section)
...
function SentinelUI:_sync_tab_state()
...
function SentinelUI:_render_tab_bar(y_start_override)
...
function SentinelUI:_render_active_tab_content(y_offset)
...
function SentinelUI:_render_section_header(section, y_offset)
...
function SentinelUI:_render_keybind_grid(section, y_offset)
...
function SentinelUI:_render_checkbox_grid(section, y_offset)
    if not section.elements or #section.elements == 0 then
        return y_offset
    end
...
function SentinelUI:_is_entry_visible(entry)
...
function SentinelUI:_get_slider_bounds(element)
    if not element then
        return 0, 100
    end
...
function SentinelUI:_apply_active_slider_from_mouse()
...
function SentinelUI:_start_key_capture(element, label)
    if not element then
        return
    end
...
function SentinelUI:_process_key_capture_input()
...
function SentinelUI:_render_key_capture_prompt()
...
function SentinelUI:_render_slider_list(section, y_offset)
    if not section.elements or #section.elements == 0 then
        return y_offset
    end
...
function SentinelUI:_render_combo_list(section, y_offset)
    if not section.elements or #section.elements == 0 then
        return y_offset
    end
...
function SentinelUI:_render_segmented_control(group, y_offset)
...
function SentinelUI:_render_progress_bar_list(section, y_offset)
    if not section.elements or #section.elements == 0 then
        return y_offset
    end
...
function SentinelUI:_render_dropdown_list(section, y_offset)
    if not section.elements or #section.elements == 0 then
        return y_offset
    end
...
function SentinelUI:_render_text_input_list(section, y_offset)
    if not section.elements or #section.elements == 0 then
        return y_offset
    end
...
function SentinelUI:_render_listbox(section, y_offset)
    if not section.elements or #section.elements == 0 then
        return y_offset
    end
...
function SentinelUI:_render_hrow(section, y_offset)
...
function SentinelUI:_render_columns(section, y_offset)
...
function SentinelUI:_render_row_list(section, y_offset)
    if not section.elements or #section.elements == 0 then
        return y_offset
    end
...
function SentinelUI:_render_metric_grid(section, y_offset)
    if not section.elements or #section.elements == 0 then
        return y_offset
    end
...
function SentinelUI:_dispatch_group_renderer(group, y_offset)
...
function SentinelUI:_render_tab_groups(section, y_offset)
...
function SentinelUI:_render_floating_tooltip()
...
function SentinelUI:_render_sections()
    self._tooltip = nil
    self._overlay_queue = {}
...
function SentinelUI:on_menu_render()
...
return {
    ...
    LAYOUT = LAYOUT,
    THEMES = THEMES,
    -- Re-export primitives so tab files can require("lib/SentinelUI") only
    color  = color,
    vec2   = vec2,
    enums  = enums,
}
