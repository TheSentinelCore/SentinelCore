...
NavBuddy uses mmtile files generated by **CMaNGOS C++ MoveMapGen** (not the Rust mmap-gen crate).
...
**Output**: ~2,746 mmtile files in `extraction/mmaps/`, copy to NavBuddy's mmaps dir.

## Water Pathfinding

NavBuddy uses water-aware polygon selection to handle lakes and oceans correctly.
...
**Reference implementations compared**: CMaNGOS (5/10 extents), AmeisenNavigation (6), BloogBot (3). NavBuddy's previous XY=50 was 10x too large, causing lake-bottom routing and water-edge stuck loops.
