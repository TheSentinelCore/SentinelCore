...
Sentinel Navigation Server is a high-performance Rust HTTP server that provides pathfinding and spatial intelligence services for World of Warcraft bots using CMaNGOS navigation mesh (mmap) files. It wraps the Recast/Detour C++ library via FFI and exposes a GET-only HTTP API consumed by Lua's `core.http_get`. Mmaps are generated by CMaNGOS C++ MoveMapGen (not the Rust mmap-gen crate).
...
6. **GET-only**: All endpoints are GET, params via query string. This is a deliberate
   convention, not a Lua limitation — `core.http_post` exists and works. Changing it means
   updating `SentinelNavClient` too.
