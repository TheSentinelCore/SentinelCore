...
/// Acquire a navmesh query pool for the given map.
/// Shared helper used by all Detour service implementations.
pub(crate) fn acquire(
    manager: &MmapManager,
    map_id: u32,
) -> Result<dashmap::mapref::one::Ref<'_, u32, detour::pool::QueryPool>, crate::error::AppError> {
    use mmap_loader::error::MmapError;
    manager.get_or_load_mesh(map_id).map_err(|e| match e {
        MmapError::MapNotFound(id) => crate::error::AppError::MapNotFound(id),
        _ => crate::error::AppError::Internal(e.to_string()),
    })?;
    manager
        .get_query_pool(map_id)
        .ok_or_else(|| crate::error::AppError::MapNotFound(map_id))
}
