...
use crate::pipeline::{
    apply_wall_clearance, has_custom_filter, create_custom_filter,
    pathfind_maybe_avoid, PathResult, SEARCH_EXTENTS, HEIGHT_EXTENTS,
};
use crate::services::{
    CoverPosition, CoverRequest, CoverResult, FleeRequest, FleeResult,
...
impl TacticalService for DetourTactical {
    ...
    fn find_cover(&self, req: CoverRequest) -> Result<CoverResult, AppError> {
        ...
        let path = if let Some(best) = positions.first() {
            let custom_filter;
            let pf_filter = if has_custom_filter(
                req.options.filter_ground,
                req.options.filter_water,
                req.options.filter_lava,
            ) {
