...
use crate::pipeline::{
    ...
    resolve_filter, parse_avoidance_zones, parse_stops, parse_waypoints,
    pathfind_maybe_avoid, pathfind_maybe_avoid_raw, post_process_path, solve_tsp,
...
pub async fn path_tsp(
    ...
    // Solve TSP (shared solver starts from node 0; rotate to start from start_idx)
    let raw_order = solve_tsp(&distances, &weights);
    let visit_order: Vec<usize> = if start_idx == 0 {
        raw_order
    } else {
        let pos = raw_order.iter().position(|&i| i == start_idx).unwrap_or(0);
        raw_order[pos..].iter().chain(raw_order[..pos].iter()).copied().collect()
    };
