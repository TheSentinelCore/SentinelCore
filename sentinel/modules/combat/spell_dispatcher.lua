...
function SpellDispatcher:queue_target(action_id, spell_id, target, queue_priority, message, opts)
    ...
    if opts.fast and type(queue.queue_spell_target_fast) == "function" then
        ...
        if ok_method then
            ...
            local ok_plain, plain_result = pcall(queue_fn, spell_id, target, queue_priority, message, opts.allow_movement ~= false)
            ok, queued, queue_method = ok_plain, plain_result, "plain_function"
        end
...
function SpellDispatcher:queue_position(action_id, spell_id, position, queue_priority, message)
    ...
    local ok, queued, queue_method = pcall(queue.queue_spell_position, queue, spell_id, position, queue_priority, message, true)
    if not ok then
        ok, queued = pcall(queue.queue_spell_position, spell_id, position, queue_priority, message, true)
        queue_method = ok and "plain_function" or "method_with_self"
    else
        queue_method = "method_with_self"
    end
    self:_set_queue_diag("position", queue_method, ok, nil)
