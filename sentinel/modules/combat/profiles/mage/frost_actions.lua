...
function Act.queue_fireball(blackboard)
    local _, target = player_and_target(blackboard)
    return queue_target(blackboard, "fireball", "fireball", target, QueuePriorities.PRIMARY)
end
...
function Act.queue_fire_blast_kill(blackboard)
    local _, target = player_and_target(blackboard)
    return queue_target(blackboard, "fire_blast_kill", "fire_blast", target, QueuePriorities.BURST)
end

function Act.queue_ice_lance_frozen(blackboard)
    local _, target = player_and_target(blackboard)
    return queue_target(blackboard, "ice_lance_frozen", "ice_lance", target, QueuePriorities.BURST)
end
...
function Act.queue_polymorph(blackboard)
    ...
    return queue_target(blackboard, "polymorph", "polymorph", best_target, QueuePriorities.UTILITY)
end

function Act.queue_polymorph_target(blackboard)
    local _, target = player_and_target(blackboard)
    return queue_target(blackboard, "polymorph_target", "polymorph", target, QueuePriorities.UTILITY)
end
...
function Act.queue_spellsteal(blackboard)
    local _, target = player_and_target(blackboard)
    return queue_target(blackboard, "spellsteal", "spellsteal", target, QueuePriorities.UTILITY)
end
...
function Act.queue_arcane_explosion(blackboard)
    local player = blackboard:get("player.object")
    return queue_target(blackboard, "arcane_explosion", "arcane_explosion", player, QueuePriorities.PRIMARY)
end
...
function Act.queue_flamestrike(blackboard)
    ...
    return queue_position(blackboard, "flamestrike", "flamestrike", pos, QueuePriorities.PRIMARY)
end
...
function Act.queue_frost_ward(blackboard)
    local player = blackboard:get("player.object")
    return queue_target(blackboard, "frost_ward", "frost_ward", player, QueuePriorities.DEFENSIVE)
end

function Act.queue_fire_ward(blackboard)
    local player = blackboard:get("player.object")
    return queue_target(blackboard, "fire_ward", "fire_ward", player, QueuePriorities.DEFENSIVE)
end

function Act.queue_summon_water_elemental(blackboard)
    local player = blackboard:get("player.object")
    return queue_target(blackboard, "summon_water_elemental", "summon_water_elemental", player, QueuePriorities.UTILITY)
end

function Act.queue_invisibility(blackboard)
    local player = blackboard:get("player.object")
    return queue_target(blackboard, "invisibility", "invisibility", player, QueuePriorities.DEFENSIVE)
end
...
function Act.queue_conjure_mana_gem(blackboard)
    ...
    for _, entry in ipairs(CONJURE_GEM_SPELLS) do
        if level >= entry.min_level then
            ...
            if spell_id then
                return queue_target(blackboard, "conjure_mana_gem", entry.key, player, QueuePriorities.MAINTENANCE)
            end
        end
    end
    return Status.FAILURE
...
function Act.use_mana_gem(blackboard)
    ...
    if type(queue.queue_item_self) == "function" then
        ok, result = pcall(queue.queue_item_self, queue, gem_id, QueuePriorities.UTILITY, "mana_gem")
        if not ok then
            ok, result = pcall(queue.queue_item_self, gem_id, QueuePriorities.UTILITY, "mana_gem")
        end
    end
