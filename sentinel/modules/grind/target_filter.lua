...
---@param all_units table[] All valid units (for cluster density)
---@return number score
local function score_target(unit, player_pos, player_level, spot, all_units, opts)
    local ok_pos, upos = pcall(unit.get_position, unit)
    ...
    -- Cluster density score (0.05): penalize targets near many other hostiles
    local nearby_count = 0
    for _, other in ipairs(all_units) do
        if other ~= unit then
            local ok_op, opos = pcall(other.get_position, other)
            if ok_op and type(opos) == "table" then
                local odist = distance_3d(ux, uy, uz, opos.x, opos.y, opos.z)
                if odist < 10 then
...
function TargetFilter.select_best(units, spot, player_level, player_pos, player, opts)
    -- Pre-filter: collect all valid targets (pcall per unit so one bad unit can't crash the scan)
    local valid = {}
    ...
    for _, unit in ipairs(valid) do
        local ok, s = pcall(score_target, unit, player_pos, player_level, spot, valid, opts)
