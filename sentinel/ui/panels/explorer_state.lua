...
function ExplorerState:set_query(q)
    q = tostring(q or "")
...
function ExplorerState.build_plan(view, bounds)
    ...
    local search_show = view.search_query ~= "" and view.search_query or "Search quests..."
    local search_color = view.search_query ~= "" and "text_primary" or "text_muted"

    push({
        kind = "rect",
        bounds = { x = text_x, y = y, w = search_w, h = CONTROL_H },
        token = "surface_raised", rounding = Theme.radius.md,
    })
    text_item("body", search_color,
        fit(search_show, search_w - Theme.space.xl), Theme.space.md,
        (CONTROL_H - Theme.line_height.body) * 0.5)
