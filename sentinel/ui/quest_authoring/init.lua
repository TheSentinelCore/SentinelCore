...
local function wrap_window(real)
  if real and rawget(real, "_qa_adapter") then return rawget(real, "_qa_adapter") end
  ...
  -- rect drawing: (x,y,w,h,color[,radius|thickness]) OR (startV, endV, color, ...)
  local function adapt_rect(method_name)
    return function(_self, a, b, c, d, e, f)
      if type(a) == "table" then
        return real[method_name](real, a, b, c, d, e)
      end
      return real[method_name](real, v2(a, b), v2(a + c, b + d), e, f)
    end
  end
  ...
  setmetatable(adapter, {
    __index = function(_t, k)
      ...
      if k == "width" or k == "height" then
        local sz = rawget(real, "get_size")
        if type(sz) == "function" then
      ...
      if type(v) == "function" then
        return function(...) return v(real, ...) end
      end
      return v
    end,
  })
  adapter.render_text = function(_self, x, y, color, text)
  ...
  if real then rawset(real, "_qa_adapter", adapter) end
  return adapter
end
...
function IDE:render(window)
    ...
    if w:is_rect_clicked(180, 3, 80, 16) then
        local res = self:_recompile()
    end
    w:render_rect_filled(180, 3, 80, 16, Theme.colors.button)
    w:render_text(186, 5, "Compile", Theme.colors.text)

    local running = self._executor ~= nil
    if w:is_rect_clicked(268, 3, 64, 16) then self:start() end
    w:render_rect_filled(268, 3, 64, 16, running and Theme.colors.border or Theme.colors.green)
    w:render_text(274, 5, "Run", Theme.colors.text)

    if w:is_rect_clicked(338, 3, 64, 16) then self:stop() end
    w:render_rect_filled(338, 3, 64, 16, running and Theme.colors.red or Theme.colors.border)
    w:render_text(344, 5, "Stop", Theme.colors.text)

    local dirtyTxt = self.ctx:isDirty() and " [unsaved*]" or " [saved]"
    w:render_text(412, 5, dirtyTxt, self.ctx:isDirty() and Theme.colors.warning or Theme.colors.text_dim)
