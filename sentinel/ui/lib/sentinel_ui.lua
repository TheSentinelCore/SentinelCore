...
function RotationSettingsUI:_render_checkbox_grid(section, y_offset)
    ...
    for i, item in ipairs(section.elements) do
        if item and item.element and self:_is_entry_visible(item) then
            ...
            -- Block window dragging only on checkbox visual area (not label text)
            self.window:is_mouse_hovering_rect_block_movement(checkbox_start, checkbox_end)
            
            if self.window:is_mouse_hovering_rect(click_start, click_end) and item.tooltip then
                self._tooltip = item.tooltip
            end
            if self.window:is_rect_clicked(click_start, click_end) then
                pcall(function()
                    if element.set then
