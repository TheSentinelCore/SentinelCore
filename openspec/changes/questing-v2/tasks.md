...
### PR1b-iii Follow-ups (deferred to a future importer slice — budget guard)

- [ ] F9 `abandon`/`fp` malformed-arg branches in `project_builder.rs` (`build_step_actions`) still drop the action silently instead of routing through `inert_preserved_action` + a diagnostic (pre-existing gap, reconfirmed out of scope in PR1b-ii and PR1b-iii; PR1b-iii hit its ~400-line budget on coverage.rs + bundle wiring + line-offset fix + block-boundary hardening first, per priority order)
