...
| 3 | Target-class filtering | Add `Action.class_restriction: Option<String>` (serde default null); `sentinel-compile --class <C>` flag (CLI-only in this change); profile named `<project>.<class>.profile.json`; excluded actions tallied in a `CompileReport` | Runtime evaluates `ClassIs` conditions | Spec requires class logic stripped at compile time; runtime stays class-agnostic; per-class profiles are self-contained. Editor API surfacing of the class param is owned by the separate in-flight `browser-editor-ui` change |
...
| 5 | Bundle split + typo tolerance | `guide_splitter` emits one `Project` per header; a canonicalization table (edit-distance-1) maps `#compltewith`→`#completewith` etc. with an info diagnostic; unknown directive/command → typed inert annotation + per-command diagnostic | Silent tolerance / drop | Never-drop policy; typo diagnostics stay grouped by command to limit noise |
