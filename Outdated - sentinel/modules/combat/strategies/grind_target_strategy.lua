local GrindTargetStrategy = {}
GrindTargetStrategy.__index = GrindTargetStrategy
...
local function is_player_unit(unit)
    ...
    if ok_player then return is_player == true end
    return false
end

function GrindTargetStrategy:new(event_bus, blackboard, izi_bridge, unit_helper)
    local o = setmetatable({}, GrindTargetStrategy)
...
function GrindTargetStrategy:is_valid_enemy(unit, opts)
    opts = opts or {}
...
function GrindTargetStrategy:_enemy_list(player_pos, opts)
    ...
    if attack_neutral then
        return self:_neutral_units_scan(player_pos, scan_radius, opts)
    end
...
function GrindTargetStrategy:_visible_enemy_list(player_pos, scan_radius, opts)
    local player = self._blackboard:get("player.object")
...
---Scan for neutral (yellow) mobs when attack_neutral is enabled.
---unit_helper.get_enemy_list_around only returns hostile (red) mobs, so we need
---to scan all objects and filter for units that are not dead and not players.
---@param player_pos table Player position for distance filtering
---@param scan_radius number Radius to search within
---@param opts table Options (include attack_neutral check)
---@return table Array of candidate units
function GrindTargetStrategy:_neutral_units_scan(player_pos, scan_radius, opts)
    if not core or not core.object_manager or type(core.object_manager.get_all_objects) ~= "function" then
        return {}
    end
    ...
    for _, candidate in ipairs(objects) do
        if candidate and self:is_valid_enemy(candidate, opts or {}) then
            -- Must be a unit (not item, not player)
            local ok_unit, is_unit = safe_call(candidate, "is_unit")
            if ok_unit and is_unit then
                -- Must not be dead (already checked in is_valid_enemy, but double-check here)
                local ok_dead, dead = safe_call(candidate, "is_dead")
                if not ok_dead or dead ~= true then
                    -- Must not be a player (already filtered in is_valid_enemy)
                    local ok_player, is_player = safe_call(candidate, "is_player")
...
function GrindTargetStrategy:_score(player, candidate, current_target, leash_center, leash_radius)
...
-- Spell helper resolution (copied from target_selector.lua from original)
...
function GrindTargetStrategy:get_best_target(opts)
    opts = opts or {}
    local player = self._blackboard:get("player.object")
