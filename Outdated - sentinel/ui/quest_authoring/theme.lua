...
local THEME = {
  ...
  -- Aliases used by panes via Theme.theme.*
  grid         = color.new(80, 90, 105, 45),  -- alias for row_sep
  error        = color.new(230, 80, 70, 255),  -- alias for text_error
  warning      = color.new(235, 200, 50, 255), -- alias for text_warn
  button       = color.new(28, 32, 40, 255),   -- alias for input_bg
  button_hover = color.new(38, 44, 52, 255),   -- alias for hover
  player       = color.new(72, 200, 110, 255),  -- alias for green
  marker       = color.new(235, 200, 50, 255),  -- alias for yellow
  marker_npc   = color.new(86, 140, 210, 255),  -- alias for accent
  path         = color.new(235, 150, 40, 255),  -- alias for orange
  cat_movement   = color.new(72, 200, 110, 255),  -- green
  cat_combat     = color.new(230, 80, 70, 255),   -- red
  cat_interaction = color.new(86, 140, 210, 255),  -- accent
  cat_quest      = color.new(235, 200, 50, 255),   -- yellow
  tooltip_bg     = color.new(14, 16, 20, 240),     -- background
  tooltip_text   = color.new(220, 225, 232, 245),  -- text
  bg_alt         = color.new(22, 26, 32, 235),     -- panel_bg
  white          = color.white(),
  timeline_block = color.new(52, 60, 72, 200),     -- panel_border
  timeline_sel   = color.new(86, 140, 210, 255),   -- accent
}
...
-- Includes both legacy names (from earlier iterations) and schema-based names.
local CATEGORY_LOOKUP = {
  ...
  -- Schema-based names (action_schema.lua)
  TravelTo = "movement", GoToGameObject = "movement", Marker = "movement",
  KillCreature = "combat",
  TalkToNPC = "interaction", Loot = "interaction", CollectItem = "interaction",
  AbandonQuest = "quest",
}
