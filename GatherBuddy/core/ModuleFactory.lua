...
ModuleFactory.register("Gather", function(cls, deps)
...
ModuleFactory.register("Mount", function(cls, deps)
    return cls:new(deps.event_bus, deps.state_machine, deps.config.movement)
end)

ModuleFactory.register("Safety", function(cls, deps)
    return cls:new(deps.event_bus, deps.state_machine, deps.config.safety)
end)

ModuleFactory.register("Inventory", function(cls, deps)
    return cls:new(deps.event_bus, deps.config.inventory)
end)

ModuleFactory.register("Statistics", function(cls, deps)
    return cls:new(deps.event_bus)
end)
